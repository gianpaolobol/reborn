# Re-born OS — Master Index

This index is the map of the Re-born repository as a product operating system.

---

## Root

- `PRODUCT.md` — master product doctrine and operating principles.
- `README.md` — public repository introduction.

---

## 01 — Foundation

- `PROJECT_OVERVIEW.md` — what Re-born is and why it exists.
- `PRINCIPLES.md` — strategic and UX principles.
- `GLOSSARY.md` — shared language.
- `DECISIONS.md` — major decisions already taken.

---

## 02 — Product Book

- `00_Product_Book_Index.md`
- `01_Vision.md`
- `02_Mission_and_Positioning.md`
- `03_Business_Model.md`
- `04_Personas.md`
- `05_Competitive_Analysis.md`
- `06_Repair_Journey_Framework.md`
- `07_Repair_Intelligence_Engine.md`
- `08_Repair_Knowledge_Graph.md`
- `09_Go_To_Market.md`
- `10_KPI_and_Metrics.md`
- `11_Risks_Assumptions.md`

---

## 03 — PRD

- `PRD_v0.1.md`
- `MVP_SCOPE.md`
- `USER_STORIES.md`
- `ACCEPTANCE_CRITERIA.md`
- `BACKLOG.md`

---

## 04 — UX Bible

- `UX_BIBLE_v0.1.md`
- `REPAIR_JOURNEY_FLOW.md`
- `SCREEN_MAP_100_PLUS.md`
- `UX_STATES_AND_EDGE_CASES.md`
- `COPYWRITING_PRINCIPLES.md`

---

## 05 — Design System

- `DESIGN_SYSTEM_INDEX.md`
- `DESIGN_TOKENS.md`
- `COMPONENT_PRINCIPLES.md`

---

## 07 — Architecture

- `APPLICATION_ARCHITECTURE.md`
- `DDD.md`
- `DOMAIN_EVENTS.md`
- `MODULE_MAP.md`

---

## 08 — Database

- `DATABASE_SCHEMA.md`
- `KNOWLEDGE_GRAPH_SCHEMA.md`

---

## 09 — API

- `API_CONTRACT.md`
- `API_ERROR_MODEL.md`

---

## 10 — Security

- `SECURITY_BASELINE.md`

---

## 14 — Roadmap

- `ROADMAP.md`
- `NEXT_ACTIONS.md`

---

## 17 — Handoff

- `MASTER_HANDOFF.md`
- `NEXT_AGENT_HANDOFF.md`


---

## Step 3 MVP Delivery Pack

- `docs/03-prd/MVP_LOCK_v0.1.md`
- `docs/03-prd/EPICS.md`
- `docs/03-prd/MVP_TICKETS.md`
- `docs/03-prd/TRACEABILITY_MATRIX.md`
- `docs/04-ux-bible/MVP_INFORMATION_ARCHITECTURE.md`
- `docs/04-ux-bible/MVP_USER_FLOWS.md`
- `docs/06-wireframes/MVP_WIREFRAMES.md`
- `docs/07-architecture/IMPLEMENTATION_PLAN.md`
- `docs/07-architecture/ROUTE_MAP.md`
- `docs/08-database/MVP_SQLITE_SCHEMA.sql`
- `docs/09-api/MVP_API_ENDPOINTS.md`
- `docs/14-roadmap/SPRINT_PLAN.md`
- `docs/14-roadmap/RELEASE_CRITERIA.md`
- `docs/18-delivery/MVP_DELIVERY_README.md`
